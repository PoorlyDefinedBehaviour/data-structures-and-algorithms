  ...
  loop {
    ...
    for visited_node in &visited_nodes {
      for unvisited_node in &unvisited_nodes {
        ...
        if cost > 0
          && cost < cheapest
          && !minimum_spanning_tree.contains(&(*visited_node, *unvisited_node, cost))
        {
          println!("{:?} {:?} {:?} {:?}", i, j, cost, minimum_spanning_tree);
    ...
    visited_nodes.push(unvisited_nodes[unvisited_node_index as usize]);
